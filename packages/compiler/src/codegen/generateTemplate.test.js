...
test('compiles the full realistic counter template with a live click handler', () => {
  ...
  test("parses a bare boolean attribute with no '=' as AttributeNode(name, true, false)", () => {
    const nodes = parseTemplate(`<input disabled />`)
    const attr = nodes[0].attributes.find((a) => a.name === 'disabled')
    assert.equal(attr.value, true)
    assert.equal(attr.dynamic, false)
  })
