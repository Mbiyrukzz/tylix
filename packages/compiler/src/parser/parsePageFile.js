...
export function parsePageFile(source) {
  ...
  const afterPageDeclaration = source.slice(pageMatch.index + pageMatch[0].length);

  const boundaries = findSectionBoundaries(afterPageDeclaration);

  const scriptEnd = boundaries.find((b) => b.keyword === "template")?.start ?? afterPageDeclaration.length;
  ...
  let template = "";
  if (templateBoundary) {
    const templateEnd = styleBoundary && styleBoundary.start > templateBoundary.start
      ? styleBoundary.start
      : afterPageDeclaration.length;
    template = afterPageDeclaration.slice(templateBoundary.contentStart, templateEnd).trim();
  }
...
function findSectionBoundaries(source) {
  const boundaries = [];
